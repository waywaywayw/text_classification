...
class ABLSTM(object):
    ...
    def build_graph(self):
        ...
        alpha = tf.nn.softmax(
            tf.matmul(tf.reshape(M, [-1, self.hidden_size]), tf.reshape(W, [-1, 1]))
        )
        r = tf.matmul(
        ...
        FC_W = tf.Variable(
            tf.truncated_normal([self.hidden_size, self.max_len], stddev=0.1)
        )
...
if __name__ == "__main__":
    ...
    x_train, y_train = load_datav2(
        "../dbpedia_data/dbpedia_csv/train.csv", sample_ratio=1e-2, one_hot=False
    )
    x_test, y_test = load_datav2("../dbpedia_data/dbpedia_csv/test.csv", one_hot=False)
    ...
    # split test_dataset to test and dev
