...
# load data
x_train, y_train = load_data("../dbpedia_data/dbpedia_csv/train.csv", sample_ratio=1)
x_test, y_test = load_data("../dbpedia_data/dbpedia_csv/test.csv", sample_ratio=1)

# data preprocessing
x_train, x_test, vocab_size = data_preprocessing_v2(
    x_train, x_test, MAX_DOCUMENT_LENGTH
)
print(vocab_size)
...
with graph.as_default():
    ...
    embeddings_var = tf.Variable(
        tf.random_uniform([vocab_size, EMBEDDING_SIZE], -1.0, 1.0), trainable=True
    )
    batch_embedded = tf.nn.embedding_lookup(embeddings_var, batch_x)
    # print(batch_embedded.shape)  # (?, 256, 100)
    rnn_outputs, _ = tf.nn.dynamic_rnn(
        BasicLSTMCell(HIDDEN_SIZE), batch_embedded, dtype=tf.float32
    )
    ...
    accuracy = tf.reduce_mean(
        tf.cast(tf.equal(prediction, tf.argmax(batch_y, 1)), tf.float32)
    )
...
with tf.Session(graph=graph) as sess:
    ...
    for e in range(epochs):
        ...
        print(
            ...
            sess.run(
                [accuracy, loss],
                feed_dict={batch_x: x_dev, batch_y: y_dev, keep_prob: 1.0},
            ),
        )
