...
class IndRNN(Classifer):
    ...
    def _build_graph(self):
        config = self.config
        # 单向rnn
        cell = IndRNNCell(config.hidden_size)
        rnn_outputs, _ = tf.nn.dynamic_rnn(cell, self.batch_embedded, dtype=tf.float32)
        ...
        W = tf.Variable(
            tf.truncated_normal([shape[1].value, config.n_class], stddev=0.1)
        )
