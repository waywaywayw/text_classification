...
class Adversarial(object):
    ...
    def build_graph(self, vocab_freq, word2idx):
        vocab_freqs = tf.constant(
            self._get_freq(vocab_freq, word2idx),
            dtype=tf.float32,
        ...
        def cal_loss_logit(embedded, keep_prob, reuse=True, scope="loss"):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                alpha = tf.nn.softmax(
                    tf.matmul(
                        tf.reshape(M, [-1, self.hidden_size]), tf.reshape(W, [-1, 1])
                    )
                )
                r = tf.matmul(
                    tf.transpose(H, [0, 2, 1]), tf.reshape(alpha, [-1, self.max_len, 1])
        ...
        logits, self.cls_loss = cal_loss_logit(
            batch_embedded, self.keep_prob, reuse=False
        )
        embedding_perturbated = self._add_perturbation(batch_embedded, self.cls_loss)
        adv_logits, self.adv_loss = cal_loss_logit(
            embedding_perturbated, self.keep_prob, reuse=True
        )
...
if __name__ == "__main__":
    # load data
    x_train, y_train = load_datav2(
        "../dbpedia_data/dbpedia_csv/train.csv", sample_ratio=1e-2
    )
    x_test, y_test = load_datav2("../dbpedia_data/dbpedia_csv/test.csv")

    # data preprocessing
    x_train, x_test, vocab_freq, word2idx, vocab_size = data_preprocessing_with_dict(
        x_train, x_test, max_len=32
    )
    print("train size: ", len(x_train))
    print("vocab size: ", vocab_size)

    # split dataset to test and dev
    x_test, x_dev, y_test, y_dev, dev_size, test_size = split_dataset(
        x_test, y_test, 0.1
    )
